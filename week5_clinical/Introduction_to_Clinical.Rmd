...
```{r}
write.csv(clinic, "/home1/jcaluya/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/jcaluya/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# The 5th patient is 50 years old.
...
# 65; the 1st patient's age at prognosis was 55, and the days to last contact was 3767, which is about 10 years.
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$lymph_nodes_examined_count, main = "Age vs. Lymph Node Count", xlab = "Age at Diagnosis", ylab = "Number of Lymph Nodes Examined", col = "red")
...
```{r Exercise 4.2}
# create plot here
boxplot(as.numeric(clinic$lymph_nodes_examined_count) ~ clinic$gender, xlab = "Gender", ylab="Number of Lymph Nodes Examined", main="Gender vs. Lymph Node Count")
```
...
# The serial time of the study/trial (in days).
...
# 15 years is approximately 5479 days, so for "Old" breast cancer patients, the survival rate is approximately 0.3/30%.
...
# 0.2; it is not statistically significant and it tells us that there is no statistically significant difference in survival between age groups, so age may not be a determining factor on its own.
